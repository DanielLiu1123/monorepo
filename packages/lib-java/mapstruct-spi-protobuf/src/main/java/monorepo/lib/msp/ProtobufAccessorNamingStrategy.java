...
public class ProtobufAccessorNamingStrategy extends DefaultAccessorNamingStrategy {
    ...
    private Map<String, Set<MethodSignature>> getInternalMethods() {
        String[] internalClasses = {
            MESSAGE, MESSAGE_BUILDER, MESSAGE_LITE, MESSAGE_LITE_BUILDER,
        };
    ...
    private List<SpecialMethodRule> stringGetterSpecialMethodRules() {
        ...
        rules.add(new SpecialMethodRule(method -> getMethodName(method).endsWith("Bytes"), (method, methods) -> {
            String methodName = getMethodName(method);
            String withoutSuffix = methodName.substring(0, methodName.length() - "Bytes".length());
            return methods.stream()
                    .anyMatch(m ->
                            getMethodName(m).equals(withoutSuffix) && isTargetClass(m.getReturnType(), String.class));
        }));
    ...
    private List<SpecialMethodRule> messageGetterGetterMethodRules() {
        ...
        rules.add(new SpecialMethodRule(method -> getMethodName(method).endsWith("Builder"), (method, methods) -> {
            String methodName = getMethodName(method);
            String withoutSuffix = methodName.substring(0, methodName.length() - "Builder".length());
            return methods.stream()
                    .anyMatch(m -> getMethodName(m).equals(withoutSuffix) && isMessageType(m.getReturnType()));
        }));

        // message field generates extra getXxxOrBuilder() method
        rules.add(new SpecialMethodRule(method -> getMethodName(method).endsWith("OrBuilder"), (method, methods) -> {
            String methodName = getMethodName(method);
    ...
    private List<SpecialMethodRule> enumGetterSpecialMethodRules() {
        ...
        rules.add(new SpecialMethodRule(method -> getMethodName(method).endsWith("Value"), (method, methods) -> {
            String methodName = getMethodName(method);
            String withoutSuffix = methodName.substring(0, methodName.length() - "Value".length());
            return methods.stream()
                    .anyMatch(m -> getMethodName(m).equals(withoutSuffix) && isProtobufEnumType(m.getReturnType()));
        }));
    ...
    private List<SpecialMethodRule> repeatedGetterSpecialMethodRules() {
        ...
        rules.add(new SpecialMethodRule(method -> getMethodName(method).endsWith("Count"), (method, methods) -> {
            String methodName = getMethodName(method);
            String withoutSuffix = methodName.substring(0, methodName.length() - "Count".length());
            return methods.stream()
                    .anyMatch(m -> getMethodName(m).equals(withoutSuffix + "List") && isListType(m.getReturnType()));
        }));

        // repeated enum field generates extra 'List<Integer> getXxxValueList()' method
        rules.add(new SpecialMethodRule(
                method -> getMethodName(method).endsWith("ValueList") && isListType(method.getReturnType()),
                (method, methods) -> {
                    String methodName = getMethodName(method);
                    String withoutSuffix = methodName.substring(0, methodName.length() - "ValueList".length());
                    return methods.stream()
                            .anyMatch(m ->
                                    getMethodName(m).equals(withoutSuffix + "List") && isListType(m.getReturnType()));
                }));

        // repeated message field generates extra getXxxBuilderList() method
        rules.add(new SpecialMethodRule(method -> getMethodName(method).endsWith("BuilderList"), (method, methods) -> {
            String methodName = getMethodName(method);
        ...
        rules.add(
                new SpecialMethodRule(method -> getMethodName(method).endsWith("OrBuilderList"), (method, methods) -> {
                    String methodName = getMethodName(method);
    ...
    private List<SpecialMethodRule> mapGetterSpecialMethodRules() {
        ...
        rules.add(new SpecialMethodRule(method -> getMethodName(method).endsWith("Count"), (method, methods) -> {
            ...
            String withoutSuffix = methodName.substring(0, methodName.length() - "Count".length());
            return methods.stream()
                    .anyMatch(m -> getMethodName(m).equals(withoutSuffix + "Map") && isMapType(m.getReturnType()));
        }));

        // map field generates deprecated 'getXxx()' and 'getMutableXxx()' methods
        rules.add(new SpecialMethodRule(
                method -> getMethodName(method).startsWith("get") && isMapType(method.getReturnType()),
                (method, methods) -> {
                    String methodName = getMethodName(method);
                    if (methodName.startsWith("getMutable")) {
                        String withoutPrefix = methodName.substring("getMutable".length());
                        return methods.stream()
                                .anyMatch(m -> getMethodName(m).equals("get" + withoutPrefix + "Map")
        ...
        rules.add(new SpecialMethodRule(
                method -> getMethodName(method).endsWith("ValueMap") && isMapType(method.getReturnType()),
                (method, methods) -> {
                    String methodName = getMethodName(method);
    ...
    private List<SpecialMethodRule> stringSetterSpecialMethodRules() {
        ...
        rules.add(new SpecialMethodRule(
                ...
                (method, methods) -> {
                    ...
                    String withoutSuffix = methodName.substring(0, methodName.length() - "Bytes".length());
                    return methods.stream()
                            .anyMatch(m -> getMethodName(m).equals(withoutSuffix)
    ...
    private List<SpecialMethodRule> messageSetterSpecialMethodRules() {
        ...
        rules.add(new SpecialMethodRule(
                ...
                (method, methods) -> {
                    String methodName = getMethodName(method);
                    return methods.stream()
                            .anyMatch(m -> getMethodName(m).equals(methodName)
